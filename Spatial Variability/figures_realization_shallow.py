...
def ground_model_data():
    path1 = "./results/shallow/Data_shallow_base_center.csv"
    path2 = "./results/shallow/Data_shallow_base_surface.csv"
    Vs_path = "./results/shallow/Vs_shallow_base.csv"

    data1 = pd.read_csv(path1)
    data2 = pd.read_csv(path2)
    Vs_base = np.loadtxt(Vs_path, delimiter=",")


    return data1, data2, Vs_base


def figures_making(path_in = "./results/shallow/", path_out="./figures/"):

    # Read all the data in the folder path_in
    files = os.listdir(path_in)

    # Do not consider the base case
    ...
    ground_data1, ground_data2, Vs_base = ground_model_data()
    ...
    for i in range(len(files_center_1)):
        ...
        # Vs
        fig_Vs_comparison(Vs_base, files_Vs_1[i], path_in, path_out)
...
def fig_surface_comparison(files_surface_1, path_in, path_out, scale=0.5, ground_data=None):
    ...
    ax[0].set_ylim(0, 6.0)
    ax[0].set_title("Surface acceleration for 2D variability")
    ...
    plt.savefig(os.path.join(path_out, "surface_comparison_shallow_{:.3f}_{:.3f}.png".format(cv, rh)), dpi=300)
    plt.close()

def fig_Vs_comparison(Vs_base, files_Vs_1, path_in, path_out, Vs2=821.1231911447858):
    # Extract data
    pattern = r"[-+]?\d*\.\d+|\d+"
    _, cv, rh = re.findall(pattern, files_Vs_1)  # extract values from one file
    cv = float(cv)
    rh = float(rh)
    ...
    Vs_base[Vs_base == Vs2] = float('nan')
    data1[data1 == Vs2] = float('nan')

    # Get dimensions of both datasets
    ymax_base, xmax_base = Vs_base.shape
    ymax_data, xmax_data = data1.shape

    # First determine the global min and max values across both datasets
    vmin = min(np.nanmin(Vs_base), np.nanmin(data1))
    vmax = max(np.nanmax(Vs_base), np.nanmax(data1))

    # Create figure and subplots
    fig, ax = plt.subplots(1, 2, figsize=(10, 5))
    
    # Define depth and position for both plots
    depth = np.arange(ymax_base)  # Assuming depth is in grid units
    
    # Create explicit extent arrays for both plots to ensure alignment
    # Format: [left, right, bottom, top]
    extent_base = [0, xmax_base, ymax_base, 0]  
    extent_data = [0, xmax_data, ymax_base, 0]  # Use same y-range as base

    im0 = ax[0].imshow(Vs_base, cmap="viridis", vmin=vmin, vmax=vmax,
                       extent=extent_base, aspect='auto')
    ax[0].set_title("Base Vs")

    im1 = ax[1].imshow(data1, cmap="viridis", vmin=vmin, vmax=vmax,
                       extent=extent_data, aspect='auto')
    
    ax[1].set_title("Vs 2D Variability")

    # You could also add y-axis labels or ticks if needed
    ...
    plt.savefig(os.path.join(path_out, "Vs_comparison_shallow_{:.3f}_{:.3f}.png".format(cv, rh)), dpi=300)
    plt.close()
...
def transfer_function_plots_comparison(files_center_1, files_Vs_1, path_in, path_out, ground_data1=None):
    ...
    # Calculate the TF
    freq_0, TF_0 = TTF.TTF(ground_data1['2'].values, ground_data1['1'].values, Vsmin=minVs, dz=1, dt=1e-5)
    freq_1, TF_1 = TTF.TTF(data_2['2'].values, data_2['1'].values, Vsmin=minVs, dz=1, dt=1e-5)

    # Plot the comparison of the transfer function
    fig, ax = plt.subplots(figsize=(10,5))
    ax.loglog(freq_0, TF_0, label="Base", linestyle="--")
    ax.loglog(freq_1, TF_1, label="2D Variability")
    ax.set_xlabel("Frequency [Hz]")
    ax.set_ylabel("Transfer function")
    ax.legend()
    ax.set_title("Transfer function comparison for CV = {:.3f} and rH = {:.3f}".format(cv, rh))
    ax.grid(True, which="both")
    plt.savefig(os.path.join(path_out, "transfer_function_comparison_shallow_{:.3f}_{:.3f}.png".format(cv, rh)), dpi=300)
    plt.close()
...
def fig_center_surface_comparison(files_center_1, path_in, path_out, ground_data=None):
    ...
    ax[0].plot(data_2['0'], data_2['2'], label="2D Variability")
    ...
    ax[1].plot(data_2['0'], data_2['1'], label="2D Variability")
    ...
    plt.savefig(os.path.join(path_out, "Central_comparison_shallow_{:.3f}_{:.3f}.png".format(cv, rh)), dpi=300)
    plt.close()


def comparison_plots(path_in = "./results/shallow/", path_out="./figures/"):
    ...
    # Read the Vs array and get the minimum
    Vsmin_vals = [np.min(pd.read_csv(os.path.join(path_in, file), header=None).values) for file in Vs_files]

    # Base case
    ground_data1, _, _ = ground_model_data()

    # Compute Ground Transfer function
    freq_0, TF_0 = TTF.TTF(ground_data1['2'].values, ground_data1['1'].values, dz=1, dt=1e-5, Vsmin=166.30786881874639)
    ...
    for j in range(len(rH_values)):
        ...
        Vs_rH = [Vsmin_vals[i] for i in index_rH]
        ...
        TF = [TTF.TTF(data[i]['2'].values, data[i]['1'].values, dz=1, dt=1e-5, Vsmin=Vsmin_vals[i]) for i in range(len(data))]
        ...
        ax.grid(True, which="both")
        plt.savefig(os.path.join(path_out, f"transfer_function_comparison_shallow_rH={rH_values[j]}.png"), dpi=300)
        plt.close()
    ...
    for j in range(len(cv_values)):
        ...
        Vs_cv = [Vsmin_vals[i] for i in index_cv]
        ...
        # Compute Transfer function
        TF = [TTF.TTF(data[i]['2'].values, data[i]['1'].values, dz=1, dt=1e-5, Vsmin=Vs_cv[i]) for i in range(len(data))]
        ...
        ax.grid(True, which="both")
        plt.savefig(os.path.join(path_out, f"transfer_function_comparison_shallow_CV={cv_values[j]}.png"), dpi=300)
        plt.close()
