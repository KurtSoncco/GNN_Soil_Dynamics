...
def variability(Vs_array, CV, rH, aHV, dx=1, dz=1, extra_z=0):
    # Extract values
    ...
    z = np.linspace(0, h+10+extra_z*dz, int((h+10)/dz+1+extra_z)) + dz/2
    ...
    y_prior = y_add_normalized*5 + h # Assuming a h_scale of 5 in the interlayer variability
    ...
    result_avg = dz/(mapping(diff)/Vs_A + (dz-mapping(diff))/Vs2)
...
def extending_array(Z_Vs_masked, x, z, dx=1):
...
def itasca_analysis(Vs_mat, G_mat, K_mat, rho_mat, X, Z, h, freq1=1.5, t01=2.5, dx=1):
    ...
    print(f"Vs base: {round(Vs_mat[0, mid_idx],4)}")
    ...
    """.format(h_total=ny*1, x_mid=x_mid, base_rec=ny*1-(h+10)))

    # Recording of surface accelerations
    x_rec = np.unique(X) - dx/2
    string_rec = "\n".join([
    "zone history acceleration-x position {i} {ny}".format(i=i, ny=Vs_mat.shape[0]*1)
    for i in x_rec
])
...
def base_case(Vs_array, dx, dz, extra_z=0):
    # Extract values
    Vs1, Vs2, h = Base_case_extraction(Vs_array)
    print(f"Vs1: {Vs1}, Vs2: {Vs2}, h: {h}")
    ...
    X = X.reshape(len(z), len(x))
    Z = Z.reshape(len(z), len(x))
    np.savetxt(f"./results/Vs_shallow_base.csv", Z_Vs_masked, delimiter=",")
    ...
    # Save results
    title_format = "Data_shallow_base"
...
def main_fun():
    ...
    for i, cv, rh in itertools.product(index, CV_list, rh_list):
        ...
        np.savetxt(f"./results/Vs_shallow_{i}_{cv}_{rh}.csv", Z_Vs_masked, delimiter=",")
        ...
        # Save results
        title_format = "Data_shallow_{:.3f}_{:.3f}_{:.3f}".format(i, cv, rh)
        save_results(title_format)
